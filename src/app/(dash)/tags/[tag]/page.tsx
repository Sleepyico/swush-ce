...
import { files, filesToTags, tags } from "@/db/schema";
...
export async function generateMetadata({ params }: { params: Params }) {
  ...
  const key = decodeURIComponent(tag || "");
  ...
  return { title: `Tag: ${folder?.name ?? "Unknown"}` };
}
...
export default async function TagFilesPage({
  ...
  return (
    <PageLayout title={`Files tagged: #${tag}`}>
      <TagFilesClient tag={tag} initialItems={rows} initialQ={q} />
    </PageLayout>
  );
}
